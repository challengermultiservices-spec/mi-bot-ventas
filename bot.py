...
def rastrear_amazon_top_10():
    ...
    try:
        ...
        for item in items:
            if len(productos_encontrados) >= 10: break # Solo queremos 10
            ...
            # FILTRO: Evitar suscripciones o planes digitales
            if any(x in nombre.lower() for x in ["subscription", "plan", "membership", "digital"]):
                continue
...
if __name__ == "__main__":
    print("🚀 Iniciando Escaneo de Top 10 para CHM Brand...")
    lista_productos, error = rastrear_amazon_top_10()

    if not lista_productos:
        enviar_telegram(f"❌ No se encontraron productos. Error: {error}")
        exit(1)
    ...
    for p in lista_productos:
        try:
            ...
            if r.status_code == 200:
                ...
                time.sleep(2) # Pausa breve para no saturar Make
        except:
            print(f"❌ Error enviando {p['producto']}")

    enviar_telegram(f"✅ *¡Éxito!* Se enviaron {enviados} productos reales a Make.")import requests
from bs4 import BeautifulSoup
import os
import time
