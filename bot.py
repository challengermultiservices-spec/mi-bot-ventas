...
def enviar_email(producto, link_video):
    ...
    try: requests.post(url, json=data, headers={"Authorization": f"Bearer {api_key}"}, timeout=10)
    except: print("⚠️ Email no enviado")
...
def enviar_sms(producto):
    ...
    try: requests.post(url, data={"To": mi_cel, "From": n_tw, "Body": f"Bot: Video listo para {producto}"}, auth=(sid, token), timeout=10)
    except: print("⚠️ SMS no enviado")
...
def ejecutar_bot_maestro():
    ...
    try:
        ...
        # Extraer JSON incluso si hay texto extra
        match = re.search(r'\{.*\}', r_g.text, re.DOTALL)
        if not match: raise Exception("IA no devolvió JSON válido")
        ...
        res_c = requests.post(u_c, headers=h_c, json=payload, timeout=30).json()
        render_id = res_c[0]['id']
        video_url = res_c[0]['url']

        # Espera de renderizado (Máximo 3 minutos)
